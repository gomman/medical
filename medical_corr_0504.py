...
crosscorr = np.zeros(x.shape[1])
for j in range(x.shape[1]):
    #x_temp = x[:,j]/np.linalg.norm(x[:,j])
    crosscorr[j],_ = np.abs(pearsonr(y_temp,x[:,j]))
print (crosscorr)
print (crosscorr.argsort()[-24:][::-1])
large_corr_idx = crosscorr.argsort()[-24:][::-1]
x = x[:,large_corr_idx]
accuracy_sum = 0
